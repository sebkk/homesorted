...
export function FinanceTabbar({
	...
	return (
		<nav
			className='absolute left-4 right-4 z-20 bg-nav-bg glass border border-border rounded-xl flex gap-1 p-[7px_8px] shadow-glass'
		...
		>
			<div
				...
				style={{
					top: '7px',
					bottom: '7px',
					left: `calc(8px + (100% - 12px) * ${activeIndex} / ${TABS.length})`,
			...
			{TABS.map(tab => {
				...
				return (
					<button
						...
						className={`relative flex-1 flex flex-col items-center gap-[3px] border-none font-sans text-[10.5px] font-semibold py-1.5 px-1 rounded-md ${
