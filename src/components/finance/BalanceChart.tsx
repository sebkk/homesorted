...
export function BalanceChart({ data, onBarClick }: { data: ChartMonth[]; onBarClick?: (month: string) => void }) {
  ...
  const maxTotal = Math.max(1, ...data.map((d) => d.income + d.expense + d.saving));
    ...
    <div className="relative">
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-auto block overflow-visible">
        ...
        {data.map((d, i) => {
          ...
          const incH = Math.max(d.income > 0 ? 3 : 0, (d.income / maxTotal) * usableH);
          const expH = Math.max(d.expense > 0 ? 3 : 0, (d.expense / maxTotal) * usableH);
          const savH = Math.max(d.saving > 0 ? 3 : 0, (d.saving / maxTotal) * usableH);
          ...
          const showTip = (clientEvt: React.MouseEvent) => {
            const rect = (clientEvt.target as SVGRectElement).getBoundingClientRect();
            const cont = (clientEvt.currentTarget as SVGSVGElement).closest(".relative")?.getBoundingClientRect();
            setTip({
          ...
          return (
            <g key={d.month} onMouseLeave={() => setTip(null)}>
              ...
              <text x={cx} y={h - 6} textAnchor="middle" fontSize={9.5} fill="var(--ink-faint)">
                {monthShort(d.month)}
              </text>
            </g>
          );
