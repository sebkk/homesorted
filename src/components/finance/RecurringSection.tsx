...
function useRecurringRange() {
  const t = useTranslations("recurring");
  const { monthIn } = useMonthFormat();
  return (tpl: Pick<RecurringTemplate, "start_month" | "end_month">): string => {
    if (!tpl.end_month) return t("rangeOpen", { from: monthIn(tpl.start_month) });
    if (tpl.end_month <= tpl.start_month) return t("rangeNever");
    return t("rangeClosed", { from: monthIn(tpl.start_month), to: monthIn(prevMonth(tpl.end_month)) });
  };
}
...
export function RecurringSection({
  ...
  return (
    <div className="flex flex-col items-stretch mb-[22px]">
      ...
      {active.length === 0 && <div className="text-[12.5px] text-ink-muted">{t("noneActive")}</div>}
      ...
      {extra.length > 0 && (
        <>
          ...
          >
            ...
            {showAll ? t("showLess") : t("showMore", { count: extra.length })}
          </button>
        </>
      )}
      {ended.length > 0 && (
...
function Row({
  ...
  const t = useTranslations("recurring");
  const recurringRange = useRecurringRange();
  ...
  return (
    <div className={`bg-surface-2 border border-border rounded-md p-3 flex items-center gap-3 ${muted ? "opacity-60" : ""}`}>
      ...
      {onDisable && (
        <IconButton label={t("disableNow")} onClick={onDisable} danger>
          <TrashIcon />
        </IconButton>
      )}
    </div>
